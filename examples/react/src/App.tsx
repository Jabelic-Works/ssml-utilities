...
const readErrorMessage = async (response: Response): Promise<string> => {
  ...
  if (contentType.includes("application/json")) {
    ...
    return payload.details ? `${payload.error}: ${payload.details}` : payload.error ?? response.statusText;
  }
...
function App() {
  ...
  const handleSynthesize = async () => {
    ...
    try {
      ...
      setStatus("error");
      setStatusText(error instanceof Error ? error.message : "音声生成に失敗しました。");
      replaceAudioUrl(null);
    }
  };
    ...
    >
      <div style={{ maxWidth: "960px", margin: "0 auto" }}>
        ...
        >
          <label style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            ...
            <input
              ...
              onChange={handleFieldChange(setSubscriptionKey, SESSION_KEYS.subscriptionKey)}
          ...
          <label style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            ...
            <select
              value={outputFormat}
              onChange={handleFieldChange(setOutputFormat, SESSION_KEYS.outputFormat)}
            >
        ...
        >
          ...
          <p style={{ marginTop: 0, color: status === "error" ? "#b42318" : "#555" }}>
